...
import { AlertCircle, CalendarClock, Clock, Edit, ExternalLink, Inbox, Users } from "lucide-react";
...
export function ProjectDetails({ project, teamMembers = [], isCreator = false, isPublicView = false }: ProjectDetailsProps) {
  ...
  const acceptApplicationMutation = useMutation({
    mutationFn: async (applicationId: number) => {
      return await apiRequest("POST", `/api/applications/${applicationId}/accept`, {});
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [`/api/projects/${project.id}/applications`] });
      queryClient.invalidateQueries({ queryKey: [`/api/projects/${project.id}/team`] });
      toast({
  ...
  const rejectApplicationMutation = useMutation({
    mutationFn: async ({ applicationId, feedback }: { applicationId: number, feedback?: string }) => {
      return await apiRequest("POST", `/api/applications/${applicationId}/reject`, { feedback });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [`/api/projects/${project.id}/applications`] });
      toast({
  ...
  const handleRejectApplication = (applicationId: number, feedback?: string) => {
    rejectApplicationMutation.mutate({ applicationId, feedback });
  };

  return (
    <>
      {isCreator && !isPublicView && applications && applications.length > 0 && (
        <Card className="mb-6">
          <CardHeader>
                    ...
                    {applications
                      ...
                      .map(application => (
                        <ApplicationCard 
                          ...
                          onReject={(feedback) => handleRejectApplication(application.id, feedback)}
                          isExpanded
                        />
                      ))
                    }
        ...
        <div className="md:col-span-2 space-y-6">
                ...
                <div>
                  ...
                  <div className="mt-1 flex items-center">
                    <Avatar className="h-6 w-6 mr-2">
                      <AvatarFallback>{creator?.name?.substring(0, 2) || creator?.username?.substring(0, 2) || "CR"}</AvatarFallback>
                      {creator?.profilePicture && <AvatarImage src={creator.profilePicture} />}
                    </Avatar>
                    <span>
                      {creator ? creator.name || creator.username : `User ${project.creatorId}`}
                    </span>
                  </div>
                </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Skills required for this project</CardDescription>
            </CardHeader>
            <CardContent>
            ...
            <Card>
              ...
              <CardContent>
                <p className="text-gray-600 dark:text-gray-300 mb-4">
                  Interested in joining this project? You can apply by clicking the "Apply to Join" button at the top of the page.
                </p>
                <p className="text-gray-600 dark:text-gray-300">
                  Be sure to highlight your relevant skills and experience in your application message.
                </p>
              </CardContent>
            </Card>
          )}
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditedProject({ ...editedProject, skillsNeeded: e.target.value })
                  }
                />
              </div>
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditedProject({ ...editedProject, timeline: e.target.value })
                  }
                />
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditedProject({ ...editedProject, website: e.target.value })
                  }
              ...
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditedProject({ ...editedProject, focus: e.target.value })
                  }
            ...
            <div className="grid gap-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setEditedProject({ ...editedProject, coverImage: e.target.value })
                }
