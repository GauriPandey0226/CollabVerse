...
export function Navbar({ transparent = false }: { transparent?: boolean }) {
  ...
  const handleScroll = () => {
    if (transparent) {
      if (window.scrollY > 10) {
        setBgClass("bg-white bg-opacity-80 dark:bg-gray-800 dark:bg-opacity-80 backdrop-blur-md border-b border-gray-200 dark:border-gray-700");
      } else {
  ...
  return (
    <nav className={`fixed w-full top-0 z-50 transition-colors duration-300 ${bgClass}`}>
        ...
        <div className="flex justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            {user ? (
              ...
              <>
                <Link href="/auth">
                  <a className="hidden sm:inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors">
                    Login
                  </a>
                </Link>
                <Link href="/auth">
